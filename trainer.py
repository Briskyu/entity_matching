...
class Trainer:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    self._maybe_log_save_evaluate(False, tr_loss, model, epoch, ignore_keys_for_eval, inputs=inputs)
                else:
            ...
            # 这个tr_loss是目前所有step的loss，执行下面这个方法会将tr_loss除以目前step数，计算出平均loss
            # 并将tr_loss置零
            self._maybe_log_save_evaluate(True, tr_loss, model, epoch, ignore_keys_for_eval, inputs=inputs)
    ...
    def _maybe_log_save_evaluate(self, is_epoch, tr_loss, model, epoch, ignore_keys_for_eval, **kwargs):
        #if self.control.should_log:
        if self.control.should_log or is_epoch:
            logs: Dict[str, float] = {}
